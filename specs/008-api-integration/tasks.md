...
### Phase 1: Static vs Dynamic Decoupling [ ]
- [ ] Create `web/src/types/constants.ts` containing pure static catalogs (`SKILL_CATALOG`, `DAYS`, `SLOTS`, `SLOT_LABELS`)
- [ ] Update UI components (`StudentSkills`, `StudentAvailability`, `FormationBoard`, `CohortWorkspace`) to import static constants from `constants.ts` instead of `data/mock.ts`
- [ ] Verify `npm run typecheck` passes after decoupling static constants

### Phase 2: API Client & Auth Wiring [ ]
- [ ] Review `web/src/api.ts` to ensure full coverage of `routes_profile.py`, `routes_cohort.py`, and `routes_formation.py` endpoints
- [ ] Add error/status helpers or loading hooks if needed across components
- [ ] Ensure `useAuth().token` is passed cleanly into `api.ts` methods across all pages
